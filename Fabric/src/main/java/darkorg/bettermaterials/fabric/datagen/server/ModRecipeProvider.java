...
public class ModRecipeProvider extends FabricRecipeProvider {
    ...
    public void buildRecipes(Consumer<FinishedRecipe> pFinishedRecipeConsumer) {
        ...
        //Misc
        addShearsRecipe(BetterMaterialsItems.COPPER_SHEARS, Items.COPPER_INGOT, pFinishedRecipeConsumer);
        ...
        //Vanilla
        addShapelessRecipe(Items.CLAY_BALL, 4, Blocks.CLAY, pFinishedRecipeConsumer);
        addShapelessTagRecipe(Items.STRING, 4, ItemTags.WOOL, pFinishedRecipeConsumer);
    ...
    static class ModRecipeBuilder {
        ...
        static void addShearsRecipe(Supplier<Item> result, ItemLike material, Consumer<FinishedRecipe> consumer) {
            ShapedRecipeBuilder.shaped(RecipeCategory.TOOLS, result.get()).define('#', material).pattern(" #").pattern("# ").unlockedBy("has_" + material, has(material)).save(consumer);
        }
        ...
        static void addShapelessRecipe(ItemLike result, int count, ItemLike ingredient, Consumer<FinishedRecipe> consumer) {
            ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, result, count).requires(ingredient).unlockedBy("has_" + result, has(result)).save(consumer);
        }

        static void addShapelessTagRecipe(ItemLike result, int count, TagKey<Item> ingredientTag, Consumer<FinishedRecipe> consumer) {
            ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, result, count).requires(ingredientTag).unlockedBy("has_" + result, has(result)).save(consumer);
        }
    }
}
